...
import (
	"database/sql"
	"errors"
	"github.com/mahendraintelops/intelops-procurement-solution-v3/invoice-service/pkg/rest/server/daos/clients/sqls"
	"github.com/mahendraintelops/intelops-procurement-solution-v3/invoice-service/pkg/rest/server/models"
...
func (invoiceDao *InvoiceDao) CreateInvoice(m *models.Invoice) (*models.Invoice, error) {
	insertQuery := "INSERT INTO invoices(Amount, PaymentTerms)values(?, ?)"
	res, err := invoiceDao.sqlClient.DB.Exec(insertQuery, m.Amount, m.PaymentTerms)
	if err != nil {
		return nil, err
	}
...
func (invoiceDao *InvoiceDao) ListInvoices() ([]*models.Invoice, error) {
	...
	for rows.Next() {
		m := models.Invoice{}
		if err = rows.Scan(&m.Id, &m.Amount, &m.PaymentTerms); err != nil {
			return nil, err
		}
...
func (invoiceDao *InvoiceDao) GetInvoice(id int64) (*models.Invoice, error) {
	...
	m := models.Invoice{}
	if err := row.Scan(&m.Id, &m.Amount, &m.PaymentTerms); err != nil {
...
func (invoiceDao *InvoiceDao) UpdateInvoice(id int64, m *models.Invoice) (*models.Invoice, error) {
	...
	updateQuery := "UPDATE invoices SET Amount = ?, PaymentTerms = ? WHERE Id = ?"
	res, err := invoiceDao.sqlClient.DB.Exec(updateQuery, m.Amount, m.PaymentTerms, id)
	if err != nil {
		return nil, err
	}
